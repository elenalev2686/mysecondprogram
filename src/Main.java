...
public class Main {
    public static void main(String[] args) {
        ...
        int addamount = 1320; // сумма пополнения счета;
        int bonus = 1; // 1 бонусный рубль за каждые 100 рублей;
        if (addamount > 1000) {
            System.out.println("Итоговый счет:");
            System.out.println((amount + addamount + ((addamount - 1000) / 100)) + " рублей");

            System.out.println("Количество бонусных рублей:");
            System.out.println(((addamount - 1000) / 100) + " бонусных рубля(ей)");
        } else {
            System.out.println("Итоговый счет:");
